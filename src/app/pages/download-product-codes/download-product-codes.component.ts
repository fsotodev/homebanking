...
@Component({
  ...
  templateUrl: './download-product-codes.component.html',
  styleUrls: ['./download-product-codes.component.scss']
})
export class DownloadProductCodesComponent implements OnInit {
  WAITING_TIME = 10;
  loading: boolean;
  formGroup: FormGroup;
  categories: any;
  ...
  async downloadFile() {
    try {
      ...
      const fileName = 'codes ' + this.formGroup.get('product').value;
  ...
  async saveLog(allCodes = false) {
    ...
    await this.firebase.addAdminLog('download-valid-product-codes', {
      categoryId,
      sku: productId,
  ...
  async createCSVFile() {
    ...
    const delimiter = ',';
    const codes = await this.firebase.getCodes(this.formGroup.get('product').value, true);
    ...
    for (const code of codes) {
      fileContent +=
        ...
        (code['technicalExpirationDate'] ? this.getParsedDate(new Date(code['technicalExpirationDate'])) : '') + '\n';
      counter++;
  ...
  private setCodeRequested(uid: string) {
    localStorage.setItem(uid, new Date().toString());
  }

  private getCodeRequested(uid: string) {
    const codeRequestedStorage = localStorage.getItem(uid);
